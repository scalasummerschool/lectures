package exercise1

/* a) Create trait `Currency` describing the properties:
 *   - `value: Int`: currency value as Int; first two digits are cents: 1.50$ => 150
 *   - `show: String`: function with arity 0 giving a String representation: 150 => "1.50$"
 * b) Create sub classes for Euro, Dollar, etc.
 * c) Add a `+` method to each subclass defining how to add the respective currency.
 */

object Traits {

  // f) create an Euro from the value and show its String
  def testShowEuro(value: Int): String = ""


  // g) add values and show result
  def testPlusEuro(a: Int, b: Int): String = ""

}

